...
/* Symbol IDs from the vocabulary (cross-checked at runtime, not at compile
 * time, since the vocab lookup is data-driven). */
...
/* Walk the input bytes, classifying each. */
...
{
    /* Internal phrase break: comma, semicolon, colon. */
    return (b == ',' || b == ';' || b == ':');
}

/* Apply prosody hints to a %text token by checking whether its source byte
 * offset falls within any hint's range. */
                         ...
                         spfy_fe_delta_t            *delta)
{
    ...
    /* Get vocab via the public accessor so this stage stays oblivious to
     * fe_t internals. */
    ...
    uint16_t cur_word_id   = 0xFFFFu;
    uint16_t cur_phrase_id = 0;
    uint32_t word_start    = 0;
    int      in_word       = 0;

    /* First pass: emit one %text token per byte; track word and phrase
     * boundaries inline. */
    for (size_t i = 0; i < n; ++i) {
        ...
        if (treat_as_letter && !in_word) {
            ...
            wt.fields[1]  = (uint16_t)(i - word_start);
        ...
        /* Phrase-final punctuation closes the current phrase and advances
         * the phrase_id for subsequent tokens. */
        if (is_phrase_final(b)) {
             ...
             * (separate USel passes), evidenced by engine prsl_slot traces
             * emitting only utt 0's slots for "Hello, world." Bump
             * phrase_id so downstream tokens land in a... */
            cur_phrase_id++;
        }
    }
    if (in_word) {
     ...
     * non-space chars into one logical token. */
    uint32_t n_text = 0;
    ...
    if (!getenv("SPFY_NO_INTONATION_BREAK")) {
         ...
         * no internal phrase break and >= MIN_WORDS_FOR_BREAK words, search
         * for the first trigger word at-or-after MIN_BREAK_POSITION. */
        uint32_t n_words = 0;
        ...
        for (uint32_t pid = 0; pid <= cur_phrase_id; ++pid) {
            ...
            /* Strict midpoint filter: trigger position must be >=
             * ceil(words/2) to exclude pre-midpoint over-fires (preposition
             * `to`, etc.). */
             ...
             * words[break_word_idx..last_word_idx] + their associated %text
             * tokens + their %token tokens to (cur_phrase_id+1). */
