...
/* Cached code region: virtually all execution stays inside the loaded
 * image, so derive the */
/* instruction host pointer with register arithmetic (base + (eip-lo))
 * instead of a per-instruction */
/* page-map array load. */
...
/* JIT x87 trampolines: reuse the EXACT interpreter C (same emcc, same
 * flags) so the JIT's FIST/FISTP and */
/* transcendentals match bit-for-bit by construction (incl. */
/* fpu_round_rc reading the live CPU.fpu_cw). */
/* index; jit_helpers (wasm_main.c) takes their addresses, which forces a
 * table slot and returns the indices. */
...
/* Safe because the code section is one contiguous host region; instructions
 * don't span regions. */
    ...
    if(m.is_mem){
        ...
        switch(op){
            ...
            case 0xDD:
                ...
                if(reg==7){ wr16(a,fpu_status()); return 1; }
                if(reg==4||reg==6){ return 1; }
                break;
            case 0xDB:
                ...
                if(reg==7){  double d=*st(0); uint64_t mant; uint16_t se; int sign=d<0; if(sign)d=-d;
        ...
        switch(op){
            ...
            case 0xDC:
                /* Same reversal as the DE pop-forms below (NOT like the DC
                 * *memory* form, where dest=st0). */
                /* Was swapped: corrupted exp()'s f2xm1 fraction -> wrong
                 * env coeffs -> sc4 gated. */
            ...
            case 0xD9:
                if(m.reg==0){ double v=*st(i); fpush(v); return 1; }
                if(m.reg==1){ double t=*st(0); *st(0)=*st(i); *st(i)=t; return 1; }
                if(m.reg==2||m.reg==3){ return 1; }
                switch(full){
                    case 0xE0:*st(0)=-*st(0);return 1;
                    case 0xE1:if(*st(0)<0)*st(0)=-*st(0);return 1;
                    case 0xE4:set_fcom(*st(0),0.0);return 1;
                    case 0xE5:{
                        double v=*st(0); uint64_t bits; memcpy(&bits,&v,8);
                        CPU.fpu_sw &= ~((1u<<8)|(1u<<9)|(1u<<10)|(1u<<14));
                        if(bits>>63) CPU.fpu_sw |= (1u<<9);
                        if(v!=v)            CPU.fpu_sw |= (1u<<8);
                        else if(v==0.0)     CPU.fpu_sw |= (1u<<14);
                        else { int e=(int)((bits>>52)&0x7ff);
                            if(e==0x7ff)    CPU.fpu_sw |= (1u<<8)|(1u<<10);
                            else if(e==0)   CPU.fpu_sw |= (1u<<10)|(1u<<14);
                            else            CPU.fpu_sw |= (1u<<10);
                        }
                        return 1; }
                    case 0xE8:fpush(1.0);return 1;
                    case 0xE9:fpush(3.321928094887362);return 1;
                    case 0xEA:fpush(1.442695040888963);return 1;
                    case 0xEB:fpush(3.141592653589793);return 1;
                    case 0xEC:fpush(0.301029995663981);return 1;
                    case 0xED:fpush(0.693147180559945);return 1;
                    case 0xEE:fpush(0.0);return 1;
                    case 0xF0:*st(0)=pow(2.0,*st(0))-1.0;return 1;
                    case 0xF1:{double y=*st(1),x=*st(0); fpop(); *st(0)=y*(log(x)/log(2.0)); return 1;}
                    case 0xF2:*st(0)=tan(*st(0));fpush(1.0);return 1;
                    case 0xF3:{double y=*st(1),x=*st(0); fpop(); *st(0)=atan2(y,x); return 1;}
                    case 0xFA:*st(0)=sqrt(*st(0));return 1;
                    case 0xFC:*st(0)=fpu_round_rc(*st(0));return 1;
                    case 0xFD:*st(0)=ldexp(*st(0),(int)*st(1));return 1;
                    case 0xFE:*st(0)=sin(*st(0));return 1;
                    case 0xFF:*st(0)=cos(*st(0));return 1;
                    case 0xD0:return 1;
                }
                if(m.reg==4||m.reg==5||m.reg==6||m.reg==7) return 1;
                break;
            case 0xDD:
                if(m.reg==0){ return 1; }
                if(m.reg==2){ *st(i)=*st(0); return 1; }
                if(m.reg==3){ *st(i)=*st(0); fpop(); return 1; }
                if(m.reg==4){ set_fcom(*st(0),*st(i)); return 1; }
                if(m.reg==5){ set_fcom(*st(0),*st(i)); fpop(); return 1; }
                break;
            case 0xDA:
                if(full==0xE9){ set_fcom(*st(0),*st(1)); fpop(); fpop(); return 1; }
                return 1;
            case 0xDB:
                if(full==0xE2){ CPU.fpu_sw=0; return 1; }
                if(full==0xE3){ CPU.fpu_top=0; CPU.fpu_sw=0; CPU.fpu_cw=0x037f; return 1; }
                if(full==0xE0||full==0xE1||full==0xE4){ return 1; }
                if(m.reg==5){ set_fcomi(*st(0),*st(i)); return 1; }
                if(m.reg==6){ set_fcomi(*st(0),*st(i)); return 1; }
                break;
            case 0xDF:
                if(full==0xE0){ setreg(EAX,2,fpu_status()); return 1; }
                if(m.reg==5){ set_fcomi(*st(0),*st(i)); fpop(); return 1; }
                if(m.reg==6){ set_fcomi(*st(0),*st(i)); fpop(); return 1; }
                break;
        }
        return 0;
    }
...
static double sse_arith(uint8_t op,double a,double b){
    switch(op){ case 0x58:return a+b; case 0x59:return a*b; case 0x5C:return a-b; case 0x5E:return a/b;
        case 0x5D:return (a<b||b!=b)?a:b;  case 0x5F:return (a>b||b!=b)?a:b;
        case 0x51:return sqrt(b); case 0x53:return 1.0/b; case 0x52:return 1.0/sqrt(b); }
    return 0;
}
static void sse_setcmp(double a,double b){
    setf(FL_OF,0); setf(FL_SF,0); setf(FL_AF,0);
    if(a!=a||b!=b){ setf(FL_ZF,1); setf(FL_PF,1); setf(FL_CF,1); }
    else if(a<b){ setf(FL_ZF,0); setf(FL_PF,0); setf(FL_CF,1); }
    else if(a==b){ setf(FL_ZF,1); setf(FL_PF,0); setf(FL_CF,0); }
...
static int do_sse(uint8_t op2,int p){
    ...
    case 0x12: case 0x16: {
        ...
        else { d->q[hi?1:0] = hi ? CPU.xmm[m.rm].q[0] : CPU.xmm[m.rm].q[1]; }
        return 1; }
    case 0x13: case 0x17: {
    ...
    case 0x28: case 0x29: {
        ...
        return 1; }
    case 0x2B: { decode_modrm(&m,0); sse_rm_write(&m,&CPU.xmm[m.reg],16); return 1; }
    case 0x2A: {
    ...
    case 0x71: case 0x72: case 0x73: {
        ...
        if(op2==0x73){
            if(sub==2){ for(int i=0;i<2;i++) d->q[i]= imm>=64?0:(d->q[i]>>imm); }
            else if(sub==6){ for(int i=0;i<2;i++) d->q[i]= imm>=64?0:(d->q[i]<<imm); }
            else if(sub==3){ xmreg_t r; memset(&r,0,16); for(int i=0;i+imm<16;i++) r.b[i]=d->b[i+imm]; *d=r; }
            else if(sub==7){ xmreg_t r; memset(&r,0,16); for(int i=15;i-imm>=0;i--) r.b[i]=d->b[i-imm]; *d=r; }
    ...
    case 0x7E: {
        ...
        return 1; }
    case 0x6F: { decode_modrm(&m,0); sse_rm_read(&m,&CPU.xmm[m.reg],16); return 1; }
    case 0x7F: { decode_modrm(&m,0); sse_rm_write(&m,&CPU.xmm[m.reg],16); return 1; }
    case 0xD6: { decode_modrm(&m,0); xmreg_t* d=&CPU.xmm[m.reg];
...
uint64_t g_insns = 0;
uint64_t g_ophist[1024];
static int g_fputrace=-1, g_ftn=0, g_ftmax=0; static uint32_t g_ftlo=0, g_fthi=0;
static int g_newbt = -1;

/* ---------------- JIT dispatch (Phase 1: plumbing + deopt; empty blocks
 * just deopt) ---------------- */
/* A compiled block is a () -> i32 WASM function in the shared
 * __indirect_function_table; it reads/writes */
/* the global CPU + linear memory directly and returns a status. */
#define JIT_OK 0
#define JIT_DEOPT 1
#define JIT_FAULT 2
...
typedef struct { uint32_t eip; int32_t slot; uint32_t count; } jitent_t;
...
void jit_reset(void){
    if(!g_jit_enabled && !g_jitmap) return;
    ...
    emu_jit_flush();   /* jitmap cleared -> tell JS to free the now-orphaned compiled-block
 * instances (null their table slots) */
}
/* JIT block linking: base of g_jitmap so compiled blocks can inline the
 * lookup (entry = base + ((eip>>1)& */
/* JITMAP_MASK)*12; eip@0, slot@4) and return_call_indirect a compiled
 * successor directly. */
/* target check makes it eviction-safe. */
...
static inline jitent_t* jit_find(uint32_t eip){
    ...
    if(e->eip != eip){ e->eip=eip; e->slot=-1; e->count=0; }
    return e;
}
...
int cpu_run(uint64_t max_insns){
    ...
    int trace = ITRACE || EMU_VERBOSE || (g_fputrace>0) || g_dumpat;
    for(uint64_t n=0; n<max_insns; n++){
        ...
        if(g_jit_enabled && g_jitmap){
            ...
            if(je->slot >= 0){
                g_jit_calls++;
                int rc = ((jit_block_fn)(uintptr_t)je->slot)();
                if(rc == JIT_OK) continue;
                g_jit_deopts++;
            } else if(je->slot == -1){
        ...
        if(g_dumpat && CPU.eip==g_dumpat && !g_dumped){ g_dumped=1;
            ...
            fprintf(stderr,"  call chain (newest last):\n");
            for(int s=0;s<512;s++){ fprintf(stderr," %08x", eipring[(eipri+s)&511]); if((s&15)==15) fprintf(stderr,"\n"); } fprintf(stderr,"\n"); }
        /* (null-call eip<0x1000 is handled by the code-region resolver
         * below: mem_region_of()==NULL -> trap) */
        if(trace){
        ...
        if((uint32_t)(eip0 - g_code_lo) >= g_code_size){
            if(win32_is_import_va(eip0)){ win32_dispatch(eip0); continue; }
        ...
        g_ip = g_code_host + (eip0 - g_code_lo);
        ...
        case 0x8D:{decode_modrm(&m,0);setreg(m.reg,opsz,m.addr);}break;
        case 0x8C:{decode_modrm(&m,0); static const uint16_t sel[8]={0x23,0x1b,0x23,0x23,0x3b,0x63,0,0}; rm_write(&m,2,sel[m.reg&7]);}break;
        case 0x8E:{decode_modrm(&m,0); (void)rm_read(&m,2);}break;
        case 0x8F:{decode_modrm(&m,0);rm_write(&m,opsz,cpu_pop32());}break;

        case 0x9B: break;
        case 0x90: break;
        case 0x91:case 0x92:case 0x93:case 0x94:case 0x95:case 0x96:case 0x97:
            { uint32_t t=CPU.r[EAX]; CPU.r[EAX]=CPU.r[op-0x90]; CPU.r[op-0x90]=t; } break;
        case 0x98: if(opsz==4) CPU.r[EAX]=(int32_t)(int16_t)(CPU.r[EAX]&0xffff); else CPU.r[EAX]=(CPU.r[EAX]&~0xffffu)|((uint32_t)(int16_t)(int8_t)(CPU.r[EAX]&0xff)&0xffff); break;
        case 0x99: if(opsz==4) CPU.r[EDX]=(CPU.r[EAX]&0x80000000)?0xffffffff:0; else setreg(EDX,2,(getreg(EAX,2)&0x8000)?0xffff:0); break;
        ...
        case 0xF5: setf(FL_CF,!(CPU.eflags&FL_CF)); break;
        ...
        case 0xFF:{decode_modrm(&m,0);switch(m.reg){
            ...
            case 6:cpu_push32(rm_read(&m,opsz));break;
            }}break;

        case 0xD7: { uint32_t a=CPU.r[EBX]+(CPU.r[EAX]&0xff)+g_seg_base; setreg(EAX,1,rd8(a)); } break;
        case 0x9C: cpu_push32(CPU.eflags); break;
        case 0x9D: CPU.eflags=(cpu_pop32()&0xcd5)|0x2; break;
        case 0x9E:{uint8_t ah=(CPU.r[EAX]>>8)&0xff; CPU.eflags=(CPU.eflags&~0xd5u)|((ah&0xd5)|0x2);}break;
        case 0x9F:{uint8_t f=CPU.eflags&0xd5; f|=0x2; CPU.r[EAX]=(CPU.r[EAX]&~0xff00u)|((uint32_t)f<<8);}break;
        case 0x60:{uint32_t sp=CPU.r[ESP];cpu_push32(CPU.r[EAX]);cpu_push32(CPU.r[ECX]);cpu_push32(CPU.r[EDX]);cpu_push32(CPU.r[EBX]);cpu_push32(sp);cpu_push32(CPU.r[EBP]);cpu_push32(CPU.r[ESI]);cpu_push32(CPU.r[EDI]);}break;
        case 0x61:{CPU.r[EDI]=cpu_pop32();CPU.r[ESI]=cpu_pop32();CPU.r[EBP]=cpu_pop32();cpu_pop32();CPU.r[EBX]=cpu_pop32();CPU.r[EDX]=cpu_pop32();CPU.r[ECX]=cpu_pop32();CPU.r[EAX]=cpu_pop32();}break;
        ...
        case 0x0F: { uint8_t op2=fetch8();
            ...
            if(sse_is(op2)){ int ssep = rep?2 : repne?3 : (opsz==2)?1 : 0; if(!do_sse(op2,ssep)) trap("sse",eip0); }
            ...
            else if(op2>=0x40 && op2<=0x4F){ decode_modrm(&m,0); uint32_t v=rm_read(&m,opsz); if(cond(op2-0x40)) setreg(m.reg,opsz,v); }
            else switch(op2){
                case 0xB6:{decode_modrm(&m,0);setreg(m.reg,opsz,rm_read(&m,1)&0xff);}break;
                case 0xB7:{decode_modrm(&m,0);setreg(m.reg,opsz,rm_read(&m,2)&0xffff);}break;
                case 0xBE:{decode_modrm(&m,0);setreg(m.reg,opsz,(uint32_t)(int32_t)(int8_t)rm_read(&m,1));}break;
                case 0xBF:{decode_modrm(&m,0);setreg(m.reg,opsz,(uint32_t)(int32_t)(int16_t)rm_read(&m,2));}break;
                case 0xAF:{decode_modrm(&m,0);int32_t a=(int32_t)getreg(m.reg,opsz),b=(int32_t)rm_read(&m,opsz);int64_t r=(int64_t)a*b;setreg(m.reg,opsz,(uint32_t)r);setf(FL_CF,(int64_t)(int32_t)r!=r);setf(FL_OF,(int64_t)(int32_t)r!=r);}break;
                case 0xA2: { uint32_t leaf=CPU.r[EAX];
                    if(leaf==0){ CPU.r[EAX]=1; CPU.r[EBX]=0x756e6547; CPU.r[EDX]=0x49656e69; CPU.r[ECX]=0x6c65746e; }
                ...
                case 0x31: CPU.r[EAX]=0;CPU.r[EDX]=0; break;
                ...
                case 0xA3:{decode_modrm(&m,0); if(BTNEW){ int32_t bit=BT_SIDX(); uint8_t byte=rd8(m.addr+(uint32_t)(bit>>3)); setf(FL_CF,(byte>>(bit&7))&1); }
                    else { uint32_t bit=getreg(m.reg,opsz)&(opsz*8-1); uint32_t v=rm_read(&m,opsz); setf(FL_CF,(v>>bit)&1); } }break;
                case 0xAB:{decode_modrm(&m,0); if(BTNEW){ int32_t bit=BT_SIDX(); uint32_t a=m.addr+(uint32_t)(bit>>3); int b=bit&7; uint8_t byte=rd8(a); setf(FL_CF,(byte>>b)&1); wr8(a,byte|(uint8_t)(1u<<b)); }
                    else { uint32_t bit=getreg(m.reg,opsz)&(opsz*8-1); uint32_t v=rm_read(&m,opsz); setf(FL_CF,(v>>bit)&1); rm_write(&m,opsz,v|(1u<<bit)); } }break;
                ...
                case 0xBC:{decode_modrm(&m,0);uint32_t v=rm_read(&m,opsz);if(v==0){setf(FL_ZF,1);}else{setf(FL_ZF,0);uint32_t i=0;while(!((v>>i)&1))i++;setreg(m.reg,opsz,i);}}break;
                case 0xBD:{decode_modrm(&m,0);uint32_t v=rm_read(&m,opsz);if(v==0){setf(FL_ZF,1);}else{setf(FL_ZF,0);uint32_t i=opsz*8-1;while(!((v>>i)&1))i--;setreg(m.reg,opsz,i);}}break;
                case 0xA4: case 0xA5:{ decode_modrm(&m,0); uint32_t cnt=((op2==0xA4)?fetch8():(CPU.r[ECX]&0xff))&31;
                    ...
                    if(cnt){ uint64_t lo=(opsz==2)?(src&0xffff):src; uint64_t wide=((uint64_t)dst<<bits)|lo; wide<<=cnt;
                        uint32_t res=(uint32_t)(wide>>bits)&SZMASK[opsz]; setf(FL_CF,(dst>>(bits-cnt))&1); rm_write(&m,opsz,res); set_szp(res,opsz); } }break;
                case 0xAC: case 0xAD:{ decode_modrm(&m,0); uint32_t cnt=((op2==0xAC)?fetch8():(CPU.r[ECX]&0xff))&31;
                    ...
                    if(cnt){ uint64_t hi=(opsz==2)?(src&0xffff):src; uint64_t dl=(opsz==2)?(dst&0xffff):dst; uint64_t wide=(hi<<bits)|dl; wide>>=cnt;
                        uint32_t res=(uint32_t)wide&SZMASK[opsz]; setf(FL_CF,(dst>>(cnt-1))&1); rm_write(&m,opsz,res); set_szp(res,opsz); } }break;
                case 0xC0:{decode_modrm(&m,0);uint32_t a=rm_read(&m,1),b=getreg(m.reg,1);uint32_t s=do_add(a,b,1);setreg(m.reg,1,a);rm_write(&m,1,s);}break;
                case 0xC1:{decode_modrm(&m,0);uint32_t a=rm_read(&m,opsz),b=getreg(m.reg,opsz);uint32_t s=do_add(a,b,opsz);setreg(m.reg,opsz,a);rm_write(&m,opsz,s);}break;
                case 0xB0:{decode_modrm(&m,0);uint32_t a=rm_read(&m,1),acc=getreg(EAX,1);do_sub(acc,a,1);if(acc==a)rm_write(&m,1,getreg(m.reg,1));else setreg(EAX,1,a);}break;
                case 0xB1:{decode_modrm(&m,0);uint32_t a=rm_read(&m,opsz),acc=getreg(EAX,opsz);do_sub(acc,a,opsz);if(acc==a)rm_write(&m,opsz,getreg(m.reg,opsz));else setreg(EAX,opsz,a);}break;
                case 0xC8:case 0xC9:case 0xCA:case 0xCB:case 0xCC:case 0xCD:case 0xCE:case 0xCF:{uint32_t v=CPU.r[op2-0xC8];CPU.r[op2-0xC8]=((v&0xff)<<24)|((v&0xff00)<<8)|((v>>8)&0xff00)|((v>>24)&0xff);}break;
                case 0x1F: decode_modrm(&m,0); break;
                case 0x18:case 0x19:case 0x1A:case 0x1B:case 0x1C:case 0x1D:case 0x1E: decode_modrm(&m,0); break;
                case 0x77: break;
                case 0xAE: decode_modrm(&m,0); break;
