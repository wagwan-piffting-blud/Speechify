...
static void test_pangram(void) {
    ...
    /* The two-syllable "over" should have phoneme 0 in syl 0 stressed, and
     * phonemes 2..3 in syl 1 unstressed. */
    {
    ...
    /* Boundary-tone-only accent strings should not raise emphasis. */
    spfy_fe_slot_t slots[64];
    fe_parsed_flatten_to_slots(&p, slots, 64);
    ASSERT_EQI("flatten.slot0.emph", slots[0].emphasis_level, 0);
    ASSERT_EQI("flatten.slot2.emph", slots[2].emphasis_level, 1);
...
static void test_chunk_seam_cleanup(void) {
     ...
     * mid-identifier and inserted 2-space padding. */
    char buf[512];
