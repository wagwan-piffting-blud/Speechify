/* SAPI 5.4 en-US phone-ID -> ARPAbet -> SPR conversion. */
...
static size_t sapi_phones_to_spr(const wchar_t *pids, char *out, size_t out_n)
{
    ...
    /* Debug: log the raw pPhoneIds bytes when SPFY_SAPI_PHONE_DEBUG is set. */
    if (getenv("SPFY_SAPI_PHONE_DEBUG")) {
        FILE *fp = spfy_sapi_debug_fopen("_sapi_phone_log.txt");
        if (fp) {
