...
int spfy_feat_table_load(const spfy_vin_t *vin, spfy_feat_table_t *out)
{
    ...
    /* Walk keys until we find "filename". */
    while (p < end) {
        ...
        if (is_filename) {
            ...
            /* Sort by stored_id ascending (insertion sort kept simple; 8118
             * entries is fast either way). */
            ...
            /* Validate density: stored_ids should be 0..entry_count-1. */
            for (uint32_t i = 0; i < entry_count; ++i) {
