...
**Just the engine, no installer.** Every release also carries `spfy_synth` on its own as a single file: `spfy_synth-<platform>-20xx.xx.xx` for each Linux and macOS platform in the table above (same platform names), `spfy_synth-macos-universal-20xx.xx.xx` which runs natively on both Apple Silicon and Intel Macs, and `spfy_synth-windows-x86-20xx.xx.xx.exe` for Windows, which runs on both 32- and 64-bit Windows. The front-end is built in, so the one file is the whole engine: fetch a voice with `spfy_synth --install-voice <name>` (see `--list-available`) and synthesize. There is no GUI and no SAPI registration. On Linux and macOS run `chmod +x` on it first. The macOS binaries are signed with a Developer ID and notarized by Apple, so a browser download runs without any quarantine workaround; the one catch is that the first launch must be online, because Gatekeeper checks the notarization with Apple at that point. Checksums are in `spfy_synth-SHA256SUMS-20xx.xx.xx.txt`.
